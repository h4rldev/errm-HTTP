...
-define(STATUS_LINES, maps:fold(fun(Code, Phrase, Acc) ->
    Acc#{Code => iolist_to_binary(["HTTP/1.1 ", integer_to_list(Code), " ", Phrase, "\r\n"])}
end, #{}, ?STATUS_CODES)).
...
build(Status, Headers, Body) ->
  ...
  iolist_to_binary([StatusLine, Headers2, ?CRLF, Body]).
...
build_headers(Status, Headers, BodySize) ->
  ...
  iolist_to_binary([StatusLine, header_lines(Hdrs, <<>>), ?CRLF]).
